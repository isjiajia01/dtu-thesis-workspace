...
function build_or_v5_1_config()
    return FreshSolver.SolverConfig(
        controller=FreshSolver.ControllerConfig(
            ...
            depot_penalty_feedback_threshold=1600.0,
        ...
        routing=FreshSolver.RoutingConfig(
            ...
            controller_feedback_target_trip2_late_only=true,
            late_bucket_start_min=810,
            trip2_late_bias_weight=5.5,
...
function main(args)
    ...
    output_path = length(args) >= 3 ? args[3] : joinpath(thesis_root, "results", "raw_runs", "herlev_multiday_or_v5_1_julia.json")

    config = build_or_v5_1_config()
