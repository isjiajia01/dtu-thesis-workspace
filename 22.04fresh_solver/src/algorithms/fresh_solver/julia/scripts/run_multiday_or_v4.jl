...
function build_or_v4_config()
    return FreshSolver.SolverConfig(
        ...
        routing=FreshSolver.RoutingConfig(
            ...
            late_bucket_start_min=780,
            trip2_late_bias_weight=7.0,
            heavy_late_bias_weight=4.5,
            staging_risk_bias_weight=3.5,
        ),
...
function main(args)
    ...
    output_path = length(args) >= 3 ? args[3] : joinpath(thesis_root, "results", "raw_runs", "herlev_multiday_or_v4_julia.json")

    config = build_or_v4_config()
