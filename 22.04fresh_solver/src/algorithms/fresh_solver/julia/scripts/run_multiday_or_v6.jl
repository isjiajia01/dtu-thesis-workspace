...
function build_or_v6_config()
    return FreshSolver.SolverConfig(
        ...
        routing=FreshSolver.RoutingConfig(
            ...
            controller_bucket_feedback_weight=1.6,
...
function main(args)
    ...
    output_path = length(args) >= 3 ? args[3] : joinpath(thesis_root, "results", "raw_runs", "herlev_multiday_or_v6_julia.json")

    config = build_or_v6_config()
