...
function build_or_v7_config()
    return FreshSolver.SolverConfig(
        ...
        routing=FreshSolver.RoutingConfig(
            ...
            controller_bucket_feedback_weight=1.5,
            late_bucket_start_min=810,
            trip2_late_bias_weight=5.0,
            heavy_late_bias_weight=2.0,
            staging_risk_bias_weight=1.25,
...
function write_csv(path, rows)
    open(path, "w") do io
        ...
        for r in rows
            vals = [
                r["date"], string(r["bucket_start_min"]), r["bucket_hhmm"], string(r["departures"]),
                string(r["picking_colli"]), string(r["picking_volume"]), string(r["staging_volume"]),
                string(r["gate_over"]), string(r["picking_colli_over"]), string(r["picking_volume_over"]),
                string(r["total_picking_over"]), string(r["staging_over"]), string(r["bucket_penalty"]), r["pressure_mode"]
            ]
...
function main()
    ...
    out_csv = joinpath(thesis_root, "results", "diagnostics", "west_or_v7_picking_buckets.csv")
    out_json = joinpath(thesis_root, "results", "diagnostics", "west_or_v7_picking_summary.json")

    config = build_or_v7_config()
    ...
    day_summary = Vector{Dict{String,Any}}()
    ...
    for planning_date in instance.start_date:Day(1):instance.end_date
        ...
        sub_instance = FreshSolver.Instance(
            name=instance.name,
        ...
        previous_penalty = diagnostics.depot_penalty
        previous_overloads = diagnostics.overload_bucket_count
        previous_pressure_mode = pressure_mode
        ...
        for bucket in diagnostics.bucket_usage
            ...
            push!(rows, Dict(
                ...
                "bucket_start_min" => bucket.bucket_start_min,
                "bucket_hhmm" => bucket_str,
                "departures" => bucket.departures,
                "picking_colli" => bucket.picking_colli,
                "picking_volume" => bucket.picking_volume,
                "staging_volume" => bucket.staging_volume,
                "gate_over" => bucket.gate_over,
                "picking_colli_over" => bucket.picking_colli_over,
                "picking_volume_over" => bucket.picking_volume_over,
                "total_picking_over" => total_pick,
                "staging_over" => bucket.staging_over,
                "bucket_penalty" => bucket.bucket_penalty,
                "pressure_mode" => pressure_mode,
            ))
            pick_sum += total_pick
            gate_sum += bucket.gate_over
            staging_sum += bucket.staging_over
            ...
            if total_pick > peak_val
                peak_val = total_pick
                peak_bucket = bucket_str
            end
        end
        push!(day_summary, Dict(
            "date" => string(planning_date),
            "pressure_mode" => pressure_mode,
            "picking_over_sum" => pick_sum,
            "gate_over_sum" => gate_sum,
            "staging_over_sum" => staging_sum,
            "overload_bucket_count" => diagnostics.overload_bucket_count,
            "depot_penalty" => diagnostics.depot_penalty,
            "peak_picking_bucket" => peak_bucket,
            "peak_picking_over" => peak_val,
            "morning_picking_over" => morning,
            "midday_picking_over" => midday,
            "late_picking_over" => late,
        ))
    end
    ...
    open(out_json, "w") do io
        JSON3.pretty(io, Dict(
            "summary_by_day" => day_summary,
            "totals" => Dict(
                "days" => length(day_summary),
                "total_picking_over" => sum(d["picking_over_sum"] for d in day_summary),
                "total_gate_over" => sum(d["gate_over_sum"] for d in day_summary),
                "total_staging_over" => sum(d["staging_over_sum"] for d in day_summary),
                "morning_picking_over" => sum(d["morning_picking_over"] for d in day_summary),
                "midday_picking_over" => sum(d["midday_picking_over"] for d in day_summary),
                "late_picking_over" => sum(d["late_picking_over"] for d in day_summary),
            ),
        ))
    end
    println(out_csv)
