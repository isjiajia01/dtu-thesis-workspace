...
function build_or_v5_5_config()
    return FreshSolver.SolverConfig(
        ...
        routing=FreshSolver.RoutingConfig(
            ...
            service_shield_relief_weight=2.0,
            shadow_price_weight=0.50,
            controller_bucket_feedback_weight=0.85,
...
function main(args)
    ...
    output_path = length(args) >= 3 ? args[3] : joinpath(thesis_root, "results", "raw_runs", "west_multiday_or_v5_5_julia.json")

    config = build_or_v5_5_config()
