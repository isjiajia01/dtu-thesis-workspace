...
function build_or_v5_4_config()
    return FreshSolver.SolverConfig(
        ...
        routing=FreshSolver.RoutingConfig(
            ...
            shadow_price_weight=0.55,
            controller_bucket_feedback_weight=0.9,
...
function main(args)
    ...
    output_path = length(args) >= 3 ? args[3] : joinpath(thesis_root, "results", "raw_runs", "west_multiday_or_v5_4_julia.json")

    config = build_or_v5_4_config()
